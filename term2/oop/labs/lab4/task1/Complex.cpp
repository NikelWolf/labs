...
Complex::Complex(string string_number) {
    const char *line_data = string_number.c_str();
    char c_s;
    if (sscanf(line_data, "(%lg %c i%lg)", &re, &c_s, &im) != 3 && sscanf(line_data, "%lg %c i%lg", &re, &c_s, &im) != 3) {
        throw runtime_error{"can not parse number"};
    }
    im = c_s == '-' ? -im : im;
}
Complex::~Complex() {}
