...
namespace labs_space {
    ...
    public:
        ...
        explicit Vector(vector_size_type size = 10):
                    tmp_vector{} {
            if (size == 0) {
        ...
        Vector(initializer_list<T> lst):
                    elements{new T[lst.size()]}, el_size{lst.size()}, tmp_vector{} {
            copy(lst.begin(), lst.end(), elements);
        }

        template<typename... TArgs>
        Vector(T head, TArgs... tail):
                    tmp_vector{} {
        ...
        Vector(const Vector<T>& v):
                    elements{new T[v.size()]}, el_size{v.size()} {
            copy(v.begin(), v.end(), elements);
        }

        Vector(Vector<T>&& v):
                    elements{v.elements}, el_size{v.size()} {
            v.elements = nullptr;
            v.el_size = 0;
        }
